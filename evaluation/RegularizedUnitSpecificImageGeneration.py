...
class RegularizedClassSpecificImageGeneration():
    ...
    def __init__(self, model, target_class, model_name):
        ...
        if not os.path.exists(f'evaluation/generated/{self.model_name}/{self.target_class}/class_{self.target_class}'):
            os.makedirs(f'evaluation/generated/{self.model_name}/{self.target_class}/class_{self.target_class}')
    ...
    def generate(self, iterations=150, blur_freq=4, blur_rad=1, wd=0.0001, clipping_value=0.1):
        ...
        for i in range(1, iterations):
            ...
            if i in np.linspace(0, iterations, 10, dtype=int):
                # Save image
                im_path = f'evaluation/generated/{self.model_name}/{self.target_class}/class_{self.target_class}/c_{self.target_class}_iter_{i}_loss_{class_loss.data.cpu().numpy()}.jpg'
        ...
        with open(f'evaluation/generated/{self.model_name}/{self.target_class}/class_{self.target_class}/run_details.txt', 'w') as f:
        ...
        os.rename(f'evaluation/generated/{self.model_name}/{self.target_class}/class_{self.target_class}',
                  f'evaluation/generated/{self.model_name}/{self.target_class}/class_{self.target_class}_blurfreq_{blur_freq}_blurrad_{blur_rad}_wd{wd}')
